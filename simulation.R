...
for (s in 1:length(Sigma2_b)){
  ...
  for (i in 1:rep){
    ...
    stats = BVS(y,X,A,alpha_true,sigma2_e_true,sigma2_b_true,beta_true,prior_sigma2e,prior_sigma2b,niter,seed)
  ...
  print(stats.summary$bvs.precision[s])
  print(stats.summary$bvs.recall[s])
  print(stats.summary$bvs.pp0[s])
  print(stats.summary$bvs.pp1[s])
}
